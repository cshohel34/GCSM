@php
    $typeLabels = [
        \App\Models\CrewProfile::class          => 'Profile / Identity',
        \App\Models\CrewDocument::class         => 'Travel Document',
        \App\Models\CrewCourse::class           => 'Certificate / Course',
        \App\Models\CrewMaritimeEducation::class=> 'Maritime Education',
        \App\Models\CrewAcademic::class         => 'Education',
        \App\Models\CrewSeaService::class       => 'Sea Service',
        \App\Models\CrewBankAccount::class      => 'Bank Account',
        \App\Models\CrewOffence::class          => 'Offence',
        \App\Models\CrewNote::class             => 'Note',
    ];
    $eventStyle = [
    ...
    $hide = ['updated_at','created_at','is_draft','oma_synced_at','deleted_at','id','crew_profile_id'];
    $fmt = function ($v) {
...
<div class="bg-white rounded-lg shadow p-4 text-sm">
    <div class="gcsm-head justify-between">
        <h3 class="font-semibold text-sm md:text-base">Edit Log — every change to this profile &amp; identity</h3>
        <span class="text-xs text-slate-400">{{ $log->count() }} {{ \Illuminate\Support\Str::plural('entry', $log->count()) }}</span>
    </div>
    <p class="text-xs text-slate-400 -mt-2 mb-2">Every field edit (identity, personal details, availability, certificates, education) is recorded here automatically with who changed it, when, and the old &rarr; new value. Entries can never be deleted.</p>
    ...
    @empty
        <div class="text-slate-400 text-xs border-t pt-2">No edits recorded yet. Any change to this profile or identity will appear here.</div>
    @endforelse
    </div>
</div>
