...
@include('accounting.reports._range', ['slot' => '<div><label class="block text-xs text-slate-400">Account</label><select name="account_id" class="border rounded px-2 py-1.5">'.collect($accounts)->map(fn($a)=>'<option value="'.$a->id.'" '.(($account&&$account->id==$a->id)?'selected':'').'>'.$a->code.' — '.e($a->name).'</option>')->prepend('<option value="">— pick —</option>')->implode('').'</select></div>'])
@if ($data)
<div class="bg-white rounded-lg shadow overflow-hidden">
    <div class="px-4 py-2 border-b font-semibold text-slate-700">{{ $account->code }} — {{ $account->name }}</div>
...
@else <div class="bg-white rounded-lg shadow p-8 text-center text-slate-400">Pick an account to view its ledger.</div> @endif
@endsection
